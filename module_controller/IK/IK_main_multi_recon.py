...
# =========================
# Left mask reconstruction net
# =========================
from img_reconstruction import LeftMaskCompletionNet
...
BASE_ROOT = r"C:/Users/ishiz/Documents/Akamine_workspace/master_thesis_2025/SMR_control_3D/IK_only_result_multi_recon"
...
def load_left_completion_net(ckpt_path: str, device):
    """
    ckpt は train_left_completion.py の保存形式（{'model_state_dict': ...}）想定
    """
    net = LeftMaskCompletionNet(in_ch=5, base=32).to(device)
    ckpt = torch.load(ckpt_path, map_location=device)
    if isinstance(ckpt, dict) and "model_state_dict" in ckpt:
        net.load_state_dict(ckpt["model_state_dict"])
    else:
        # 念のため、state_dict直保存にも対応
...
@torch.no_grad()
def reconstruct_left_mask(cam_mask3: np.ndarray, q_map: np.ndarray, leftnet, device):
    """
    cam_mask3: (3,H,W) float32 0..1
    q_map    : (3,H,W) float32 0..1  （元コードの v/5.0 をそのまま使う）:contentReference[oaicite:1]{index=1}

    入力  : [center/right(2ch) + q_map(3ch)] => 5ch
    出力  : left logits(1ch) を sigmoid して left を再構成
    """
    if leftnet is None:
        return cam_mask3

    cam_mask3 = cam_mask3.astype(np.float32)
    q_map = q_map.astype(np.float32)

    cam_mask2 = cam_mask3[1:3]                  # (2,H,W)  ※left欠損を想定（trainと同じ）:contentReference[oaicite:2]{index=2}
    x5 = np.concatenate([cam_mask2, q_map], 0)   # (5,H,W)

    x5_t = torch.from_numpy(x5).unsqueeze(0).to(device)  # (1,5,H,W)
    left_logits = leftnet(x5_t)                            # (1,1,H,W)
    left_prob = torch.sigmoid(left_logits)[0, 0].detach().cpu().numpy().astype(np.float32)

    cam_recon = cam_mask3.copy()
    cam_recon[0] = np.clip(left_prob, 0.0, 1.0)
    return cam_recon
...
def run_feedback_loop_3d(
    ...
    for loop in range(num_loops):
        ...
        for m_idx in range(eff):
            ...
            # --- 追加：left欠損→再構成 ---
            cam_mask3 = reconstruct_left_mask(cam_mask3, q_map, left_completion_net, device)
...
def main():
    ...
    # =========================
    # Left mask completion net（追加）
    # =========================
    LEFT_COMPLETION_CKPT = r"C:/Users/ishiz/Documents/Akamine_workspace/master_thesis_2025/SMR_control_3D/module_controller/IK/1module_babbling_data/normal/roi_aug_shift5_rot30_beammask_3ch/training_logs_left_completion/left_completion_epoch200.pth"  # 200epoch重みへのパスに合わせて変更
    left_completion_net = None
    if os.path.exists(LEFT_COMPLETION_CKPT):
        left_completion_net = load_left_completion_net(LEFT_COMPLETION_CKPT, device)
        print(f"[INFO] Loaded LeftMaskCompletionNet: {LEFT_COMPLETION_CKPT}")
    else:
        print(f"[WARN] LeftMaskCompletionNet ckpt not found: {LEFT_COMPLETION_CKPT}")
    ...
    for trial in range(N_TRIALS):
        ...
        mse_fb, ov_fb, _ = run_feedback_loop_3d(
            ...
            left_completion_net=left_completion_net,
        )
