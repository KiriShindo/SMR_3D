...
BASE_ROOT = r"C:/Users/ishiz/Documents/Akamine_workspace/master_thesis_2025/SMR_control_3D/IK_only_result"
...
# もしあなたのdatasetで順序が違うなら、ここだけ変えてください。
CLASS_LEFT = 0
CLASS_CENTER = 1
CLASS_RIGHT = 2
...
def masks3ch_from_segnet(img_bgr, seg_predictor, score_thresh=0.0):
    """
    Returns:
      ok: bool
    ...
    # 各クラスごとに「スコア最大の1枚」を採用（重複は max 合成）
...
def run_feedback_loop_3d(
    ...
    for m_idx in range(num_modules):
        ...
        ok, mask3, overlay, info = masks3ch_from_segnet(crop, seg3_predictor)
        if not ok:
    ...
    for loop in range(num_loops):
        ...
        for m_idx in range(eff):
            ...
            ok, cam_mask3, cam_overlay, info = masks3ch_from_segnet(cam_crop, seg3_predictor)
            if not ok:
